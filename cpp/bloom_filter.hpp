...
template <size_t CounterWidth> class bloom_filter {
public:
  typedef packed::vector<size_t, CounterWidth> bitmap_type;

  struct traits {
    traits() : cells(0), hashes(0) {}
  ...
  explicit bloom_filter(const traits& traits)
      : m_traits(traits), m_bitmap(m_traits.cells) {}

  explicit bloom_filter(size_t capacity, double error_rate)
      : bloom_filter(traits(capacity, error_rate)) {}

public:
  size_t cells() const { return m_traits.cells; }

  size_t hashes() const { return m_traits.hashes; }

  const bitmap_type& bitmap() const { return m_bitmap; }

public:
  void clear() { m_bitmap.reset(); }

  template <typename Hasher> bool add(const Hasher& hasher) {
    bool overflow = false;
  ...
  template <typename Hasher> size_t lookup(const Hasher& hasher) const {
