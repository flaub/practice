...
template <class K, class V, class HashFn = hash<K>> class HashTable {
public:
  typedef K key_type;
...
public:
  HashTable() : m_buckets(64) {}

  pointer end() { return nullptr; }

  const_pointer end() const { return nullptr; }
  ...
  pointer find(const key_type& key) { return find_(key).second; }

  const_pointer find(const key_type& key) const { return find_(key).second; }

  double load_factor() const { return load_factor_(m_entries); }

  size_t size() const { return m_entries; }

  size_t bucket_count() const { return m_buckets.size(); }

  size_t bucket_size(size_t n) const { return m_buckets[n].size(); }

  size_t bucket(const key_type& key) const {
    return m_hasher(key) % bucket_count();
  }

  double avg_collisions() const { return (double)m_entries / (double)m_used; }
