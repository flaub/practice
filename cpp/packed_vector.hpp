...
namespace packed {
namespace detail {
template <class T, class C> struct base_accessor {
  typedef typename T::element_type element_type;
  typedef typename T::value_type value_type;

  base_accessor(C& container, size_t pos)
      : container(container), bit_offset(pos * T::value_width),
...
struct accessor : public base_accessor<T, C> {
  ...
  operator typename T::value_type() const {
    ...
    auto raw = low ^ ((low ^ high) &
                      (this->mask >> this->available << this->available));
    return typename T::value_type(raw);
  }
  ...
  void operator=(const typename T::value_type& value) const {
    ...
    do {
      ...
      auto new_value =
          (old_value & ~(this->mask >> (T::element_width - this->start))) |
          high;
...
struct accessor<T, C, true> : public base_accessor<T, C> {
  inline accessor(C& container, size_t pos)
      : base_accessor<T, C>(container, pos) {}

  inline operator typename T::value_type() const {
    auto element = this->container[this->index];
    auto raw = (element >> this->start) & this->mask;
    return typename T::value_type(raw);
  }

  inline void operator=(const typename T::value_type& value) const {
    typedef typename T::element_type element_type;
    auto native_value = static_cast<element_type>(value);
    auto append = (native_value << this->start);
    auto probe = &this->container[this->index];
  ...
  inline bool safe_increment() const {
    auto probe = &this->container[this->index];
    while (true) {
      volatile auto old_value = *probe;
...
template <typename T, size_t W> class vector {
public:
  ...
  typedef const detail::accessor<this_type, const container_type, power_of_2()>
      const_accessor;

public: // Member functions
  explicit vector() : m_size(0) {}

  explicit vector(size_t count, const T& value = T())
      : m_container(required_elements(count)), m_size(count) {}

  template <typename InputIt> vector(InputIt first, InputIt last) {
    assign(first, last);
  }

  vector(const this_type& rhs) : m_container(rhs), m_size(rhs.m_size) {}

  vector(this_type&& rhs) : m_container(std::move(rhs)), m_size(rhs.m_size) {}

  vector(std::initializer_list<T> init) : m_size(0) { assign(init); }
...
public: // Element acesss
  accessor at(size_t pos) { return accessor(m_container, pos); }
  ...
  accessor operator[](size_t pos) { return accessor(m_container, pos); }

  accessor front() { return accessor(m_container, 0); }

  constexpr const_accessor front() const {
    return const_accessor(m_container, 0);
  }

  accessor back() { return accessor(m_container, size() - 1); }
...
public: // Capacity
  ...
  size_t max_size() const { return m_container.max_size() / value_width; }
  ...
  static size_t max_value() { return (1 << value_width) - 1; }
